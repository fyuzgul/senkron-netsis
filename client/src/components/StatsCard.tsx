...
const StatsCard: React.FC<StatsCardProps> = ({ data, loading }) => {
  ...
  const totalAmount = data.reduce((sum, item) => sum + (item.GENELTOPLAM || 0), 0);
  const totalBrutAmount = data.reduce((sum, item) => sum + (item.BRUTTUTAR || 0), 0);
  const totalDovizAmount = data.reduce((sum, item) => sum + (item.DOVIZTUTAR || 0), 0);
  ...
  const stats = [
    ...
    {
      name: 'Brüt Toplam',
      value: formatCurrency(totalBrutAmount),
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
